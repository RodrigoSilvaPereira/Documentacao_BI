...
interface MedidaCardProps {
  medida:       MedidaDAX;
  kpis:         KPI[];
  todasMedidas: MedidaDAX[];
  onEdit:       (medida: MedidaDAX) => void;
  onDelete:     (id: string) => void;
  onDuplicate?: (id: string) => void;
}

export function MedidaCard({ medida, kpis, todasMedidas, onEdit, onDelete, onDuplicate }: MedidaCardProps) {
  ...
  const formulaPreview = medida.formula.split('\n').find((l) => l.trim() !== '') ?? '';
    ...
    <div className="bg-white border border-slate-200 rounded-xl p-5 hover:border-slate-300 transition-colors group">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          {medida.tabela && (
            <p className="text-xs font-mono text-slate-400 leading-none mb-1">{medida.tabela}</p>
          )}
          <h3 className="text-sm font-semibold text-slate-800">{medida.nome}</h3>
        </div>
        <div className="flex gap-1 flex-shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
          {onDuplicate && (
            <button onClick={() => onDuplicate(medida.id)} aria-label="Duplicar Medida"
              title="Duplicar"
              className="p-1.5 text-slate-400 hover:text-brand-600 hover:bg-brand-50 rounded-md transition-colors">
              <Copy size={14} />
            </button>
          )}
          <button onClick={() => onEdit(medida)} aria-label="Editar medida"
            className="p-1.5 text-slate-400 hover:text-brand-600 hover:bg-brand-50 rounded-md transition-colors">
            <Pencil size={14} />
          </button>
          <button onClick={() => onDelete(medida.id)} aria-label="Excluir medida"
        ...
        <div className="mt-3 pt-3 border-t border-slate-100 space-y-1.5">
          {depNomes.length > 0 && (
            <div className="flex flex-wrap items-center gap-1.5">
              ...
              {depNomes.map((nome, i) => (
                <span key={i} className="text-xs px-1.5 py-0.5 bg-purple-50 text-purple-700 rounded font-mono">
                  [{nome}]
                </span>
              ))}
            </div>
