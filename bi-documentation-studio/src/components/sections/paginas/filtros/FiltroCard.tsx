  ...
  return (
    <div className={cn(
      ...
      <div className="flex items-start gap-2.5 min-w-0 flex-1">
        <Badge variant="purple" className="flex-shrink-0 mt-0.5">
          {LABELS_TIPO_FILTRO[filtro.tipo]}
        </Badge>
        <div className="min-w-0">
          ...
          {filtro.visuais_afetados.length > 0 && (
            <p className="text-xs text-slate-400 mt-0.5">
              Afeta {filtro.visuais_afetados.length}{' '}
              {filtro.visuais_afetados.length === 1 ? 'visual' : 'visuais'}
            </p>
          )}
        </div>
      </div>
